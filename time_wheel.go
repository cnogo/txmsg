...
type (
	...
	TimingWheel struct {
		...
		ticker        *time.Ticker
		slots         []*list.List
		timers        map[interface{}]interface{}   // 时间轮内部单线程安全
		tickedPos     int
		numSlots      int
		execute       Execute
...
func NewTimingWheel(interval time.Duration, numSlots int, execute Execute) (*TimingWheel, error) {
	...
	return newTimingWheelWithClock(interval, numSlots, execute)
}
...
func (tw *TimingWheel) setTask(task *timingEntry) {
	...
	if val, ok := tw.timers[task.key]; ok {
